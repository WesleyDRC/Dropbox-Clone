class DropBoxController {
  constructor() {
    ...
    this.changeInSelection = new Event("changeselection");
  ...
  connectFirebase() {
    const firebaseConfig = {
      apiKey: "AIzaSyCcH3GdnKPmewfa4OoVWrfqIUE5h8jmRKI",
      authDomain: "dropbox-clone-4cfc1.firebaseapp.com",
      databaseURL: "https://dropbox-clone-4cfc1-default-rtdb.firebaseio.com",
      projectId: "dropbox-clone-4cfc1",
      storageBucket: "dropbox-clone-4cfc1.appspot.com",
      messagingSenderId: "405323168340",
      appId: "1:405323168340:web:e7661b58d5c79bba3fcbc1",
      measurementId: "G-PMKZ1HXE68",
    };
  ...
  ajax(
    ...
    return new Promise((resolve, reject) => {
      ...
      ajax.open(method, url); // Abre conexão via {method} & manda para pasta upload
      ...
      // Capturar hora para saber que horas começou o upload
  ...
  removeFile(ref, name) {
    // referência do arquivo que desejo excluir
    ...
    this.getSelection().forEach((li) => {
      ...
      promises.push(
        new Promise((resolve, reject) => {
          if (file.type === "folder") {
            this.removeFolder(this.currentFolder.join("/"), file.name)
              .then(() => {
                resolve({
                  fields: {
                    key,
  ...
  initEvents() {
    ...
    this.listFilesEl.addEventListener("changeselection", () => {
  ...
  formatTime(duration) {
    let seconds = parseInt((duration / 1000) % 60); // Passando de milissegundos para segundos e pegando o resto da divisão por 60
    let minutes = parseInt((duration / (1000 * 60)) % 60); // Passando de milissegundos para minutos
    let hours = parseInt((duration / (1000 * 60 * 60)) % 24); // Passando de milissegundos para horas
  ...
  uploadProgress(event, file) {
    let timeSpent = Date.now() - this.startUploadTime; // Tempo restante
    let loaded = event.loaded;
    let total = event.total;

    // Total = Regra de três -> Total = ( 100% ), Quantos MB enviados = ( X )

    let porcent = parseInt((loaded / total) * 100);

    let timeLeft = ((100 - porcent) * timeSpent) / porcent; // Quantos % faltam * Multiplicado pelo tempo que já passou / Regra de três
  ...
  renderBrowseLocation() {
    ...
    for (let i = 0; i < this.currentFolder.length; i++) {
      ...
      if (i + 1 === this.currentFolder.length) {
        // Para saber se é o ultimo elemento do array
  ...
  initEventsLi(li) {
    ...
    li.addEventListener("click", (e) => {
      if (e.shiftKey) {
        ...
        this.listFilesEl.dispatchEvent(this.changeInSelection); // Dispara evento
